...
docker exec $(docker ps | grep "$DIR"_postgres_1 | awk '{print $11}') "/bin/bash" "/setup/install.sh"
docker exec $(docker ps | grep "$DIR"_mongo_1 | awk '{print $11}') "/bin/bash" "/setup/install.sh"
